        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {features.map((f, i) => (
            <div
              key={f.title}
              data-stagger
              className="group rounded-lg border border-border glass glass-hover light-sweep p-6 text-center space-y-3 transition-all duration-500 ease-in-out hover:border-accent/40 hover:glow-sm hover:-translate-y-2 opacity-0"
              style={{ animationDelay: `${i * 100}ms` }}
            >
              <div className="mx-auto w-12 h-12 rounded-full bg-accent/10 flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:bg-accent/20">
                <f.icon className="h-5 w-5 text-accent transition-all duration-300 group-hover:drop-shadow-[0_0_8px_hsl(45,100%,60%)]" />
              </div>
